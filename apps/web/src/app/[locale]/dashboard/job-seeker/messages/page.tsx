...
import { Search, Send, MoreVertical, ArrowLeft, Loader2, Briefcase, Building2, Paperclip, X, Lock, MapPin, Clock, Banknote } from 'lucide-react';
...
const loc = {
  de: {
    title: 'Nachrichten', searchPlaceholder: 'Konversationen suchen...',
    emptyTitle: 'Keine Konversation ausgewählt', emptyDescription: 'Wähle eine Konversation aus der Liste, um zu schreiben.',
    noConversations: 'Noch keine Nachrichten', noConversationsDesc: 'Sobald du jemanden kontaktierst, erscheinen deine Gespräche hier.',
    messagePlaceholder: 'Nachricht eingeben...', moreOptions: 'Weitere Optionen',
    browseJobs: 'Jobs durchsuchen', attachFile: 'Datei anhängen',
  ...
  en: {
    ...
    noConversations: 'No messages yet', noConversationsDesc: 'Once you contact someone, your conversations will appear here.',
    messagePlaceholder: 'Type your message...', moreOptions: 'More options',
    browseJobs: 'Browse jobs', attachFile: 'Attach file',
    filterAll: 'All', filterUnread: 'Unread',
    premiumCta: 'Unlock Premium to read full messages', premiumBtn: 'Go Premium',
  },
  fr: {
    title: 'Messages', searchPlaceholder: 'Rechercher des conversations...',
    emptyTitle: 'Aucune conversation sélectionnée', emptyDescription: 'Sélectionnez une conversation dans la liste pour écrire.',
    noConversations: 'Pas encore de messages', noConversationsDesc: 'Dès que vous contactez quelqu\'un, vos conversations apparaîtront ici.',
    messagePlaceholder: 'Écrire un message...', moreOptions: 'Plus d\'options',
    browseJobs: 'Parcourir les jobs', attachFile: 'Joindre un fichier',
    filterAll: 'Tous', filterUnread: 'Non lus',
    premiumCta: 'Passez Premium pour lire les messages complets', premiumBtn: 'Devenir Premium',
  },
  it: {
    title: 'Messaggi', searchPlaceholder: 'Cerca conversazioni...',
    emptyTitle: 'Nessuna conversazione selezionata', emptyDescription: 'Seleziona una conversazione dalla lista per scrivere.',
    noConversations: 'Ancora nessun messaggio', noConversationsDesc: 'Una volta contattato qualcuno, le tue conversazioni appariranno qui.',
    messagePlaceholder: 'Scrivi un messaggio...', moreOptions: 'Altre opzioni',
    browseJobs: 'Sfoglia i lavori', attachFile: 'Allega file',
  ...
  sq: {
    title: 'Mesazhet', searchPlaceholder: 'Kërko biseda...',
    emptyTitle: 'Asnjë bisedë e zgjedhur', emptyDescription: 'Zgjidhni një bisedë nga lista për të shkruar.',
    noConversations: 'Ende pa mesazhe', noConversationsDesc: 'Sapo të kontaktoni dikë, bisedat tuaja do të shfaqen këtu.',
    messagePlaceholder: 'Shkruaj mesazhin...', moreOptions: 'Opsione të tjera',
    browseJobs: 'Shfleto punët', attachFile: 'Bashkëngjit skedar',
...
function MessagesContent() {
  ...
  const [profileData, setProfileData] = useState<{ bio?: string; location?: string; companyName?: string; createdAt?: string } | null>(null);
  const [profileAds, setProfileAds] = useState<{ id: string; category: string; salary?: string; salaryType?: string; locationCity?: string; status: string; createdAt: string; currency?: string }[]>([]);
  const messagesEndRef = useRef<HTMLDivElement>(null);

  const showMessages = getJobSeekerConfig().features.showMessages;
  ...
  const openProfile = async () => {
    ...
    try {
      ...
      if (res.ok) setProfileData(res.profile);
    } catch {}
    try {
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline justify-between gap-2 mb-0.5">
                    ...
                    <span className="text-[11px] text-slate-400 whitespace-nowrap shrink-0 tabular-nums text-right">{timeAgo(conv.lastAt, locale)}</span>
                  </div>
            ...
            <div className="flex items-end gap-4">
              ...
              <div className="pb-1 min-w-0">
                ...
                {profileData?.location && (
                  <p className="flex items-center gap-1 text-xs text-slate-400"><MapPin size={11} /> {profileData.location}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto border-t border-slate-100">
            <div className="px-6 py-4">
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">
                {{ de: 'Stellenangebote', en: 'Job Listings', fr: 'Offres d\'emploi', it: 'Offerte di lavoro', sq: 'Oferta pune' }[locale]} {profileAds.length > 0 && `(${profileAds.length})`}
              </h4>
              {profileAds.length === 0 ? (
                <div className="text-center py-6">
                  <Briefcase size={20} className="text-slate-200 mx-auto mb-2" />
                  <p className="text-xs text-slate-400">{{ de: 'Keine Stellenangebote', en: 'No job listings', fr: 'Aucune offre d\'emploi', it: 'Nessuna offerta di lavoro', sq: 'Asnjë ofertë pune' }[locale]}</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {profileAds.map((ad) => (
                    <div key={ad.id} className={clsx(
                      ...
                      <div className="flex items-center gap-2.5 text-[11px] text-slate-500">
                        ...
                        {ad.salary && <span className="flex items-center gap-1 font-medium text-[#0B1F44]/70"><Banknote size={10} className="text-slate-400" />{ad.salary}{ad.currency === 'CHF' ? ' CHF' : ''}{ad.salaryType === 'monthly' ? '/Mt.' : ''}</span>}
                      </div>
                    </div>
                  ))}
                </div>
